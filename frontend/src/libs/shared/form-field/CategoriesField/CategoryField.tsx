...
const CategoryField = ({ categoryName }: CategoryField) => {
  ...
  const [disabled, setDisabled] = useState(
    !!!getValues(`categories.${categoryName}`),
  )
  ...
  const errorMessage = Object.values(CategoryValueField)
    .map(
      (fieldName) =>
        getFieldState(`categories.${categoryName}.${fieldName}`)?.error?.message,
    )
    .find(Boolean)
    ...
    <div className={cx('container')}>
      <div className={cx('toggleContainer')}>
        <button onClick={handleToggleDisabled} type="button">
          <Icon
            name={disabled ? 'unchecked' : 'checked'}
            width={20}
            height={20}
            alt={`카테고리 인풋 ${disabled ? '비활성화' : '활성화'} 아이콘`}
          />
        </button>
        <div className={cx('label')}>
          {STYLE_CATEGORY_TITLE_MAP[categoryName]}
        </div>
      </div>
      <div>
        ...
        <div className={cx('inputsContainer')}>
          <Controller
            ...
            defaultValue={
              '' as PathValue<
                StyleFormInput,
                `categories.${CategoryKey}.${CategoryValueField.name}`
              >
            }
            render={({ field }) => (
              <TextField {...field} width="160px" placeholder="의상명" />
            )}
          />
          <Controller
            ...
            defaultValue={
              '' as PathValue<
                StyleFormInput,
                `categories.${CategoryKey}.${CategoryValueField.brand}`
              >
            }
            render={({ field }) => (
              <TextField {...field} width="160px" placeholder="브랜드명" />
            )}
          />
          <Controller
            ...
            defaultValue={
              '' as PathValue<
                StyleFormInput,
                `categories.${CategoryKey}.${CategoryValueField.price}`
              >
            }
